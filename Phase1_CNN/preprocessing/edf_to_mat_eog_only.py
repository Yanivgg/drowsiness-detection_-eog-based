...
for fname in os.listdir(data_dir):
    if fname.endswith("_annotations.edf"):
        ...
        try:
            ...
            annotations = f.readAnnotations()  # מחזיר tuple (onset, duration, description)
...
print(f"✅ נמשכו {len(df)} אירועים מתוך {df['file'].nunique()} קבצי אנוטציה")
print(df.head(20))
...
print(f"📊 סיכום קובץ: {os.path.basename(signal_file)}")
print(f"סה״כ אנוטציות: {n_events}")
print(f"סה״כ דגימות מתויגות כ־1: {n_label_ones}")
print(f"שקול לאורך של: {seconds_labeled:.1f} שניות של עייפות (מתוך {n_samples/fs:.1f} שניות סה״כ)")
...
E1, E2 = resampled_data[:2]  # או לפי שמות הערוצים הרלוונטיים
...
plt.title("השוואת אות EOG לפני ואחרי Resample ל-200 Hz")
plt.xlabel("Time (s)")
...
print("ΔPSD (area-normalized) =", np.abs(Pxx128_n[:len(Pxx200_n)] - Pxx200_n).mean())
...
fs_target = 200            # קצב הדגימה החדש
window_event_sec = 2       # 2 שניות סביב כל אנוטציה
...
for fname in tqdm(edf_files, desc="Processing EDF files"):
    ...
    # בחר שני ערוצי EOG בלבד (התאם לשמות אצלך)
